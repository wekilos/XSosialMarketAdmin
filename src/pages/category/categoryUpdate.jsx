...
const CategoryUpdate = () => {
  ...
  const updateCategory = () => {
    ...
    formData.append("has_product", category.has_product ? 1 : 0);
    formData.append("description", category.description);
    newFile && formData.append("icon", file);

    (file || !newFile) && category.title?.length > 0
      ? axiosInstance
          .post("/categories/update/" + id, formData)
          .then((data) => {
            setLoading(true);
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-[14px] font-[400]">
            {category?.updated_at?.slice(0, 10) +
              " / " +
              category?.updated_at?.slice(11, 16)}
          </h1>
        </div>
