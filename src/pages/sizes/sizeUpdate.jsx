...
const SizeUpdate = () => {
  ...
  const updateSize = () => {
    ...
    formData.append("title", size.title);
    formData.append("is_active", size.is_active ? 1 : 0);

    if (size.title?.length > 0) {
      axiosInstance
        .post("/sizes/update/" + id, formData)
        .then((data) => {
          setLoading(true);
          console.log(data.data);
          history.push({ pathname: "/sizes" });
        })
        .catch((err) => {
          setLoading(true);
          console.log(err);
  ...
  return loading ? (
    ...
    <div className="w-full">
      ...
      <div className="w-full pb-[30px] flex justify-between items-center">
        <h1 className="text-[30px] font-[700]">Razmerler</h1>
      </div>

      <div className="w-full min-h-[60vh] p-5 bg-white rounded-[8px]">
        <div className=" flex items-center gap-4 pb-5 border-b-[1px] border-b-[#E9EBF0]">
          <div className="border-l-[3px] border-blue h-[20px]"></div>
          <h1 className="text-[20px] font-[500]">Razmer maglumaty</h1>
        </div>

        <div className="flex items-center border-t-[1px] justify-between py-[30px]">
          <div className="w-[380px]">
            <h1 className="text-[18px] font-[500]">Razmer ady</h1>
            <p className="text-[14px] mt-2 font-[500] text-[#98A2B2]">
              Razmeriň adyny girizeniňizde 30 simwoldan geçmäň.
            </p>
          </div>
          <div className="flex justify-start w-[550px]">
            <input
              value={size.title}
              onChange={(e) => {
                setSize({ ...size, title: e.target.value });
              }}
        ...
        <div className="flex items-center border-t-[1px] justify-between py-[30px]">
          <div className="w-[380px]">
            ...
            <p className="text-[14px] mt-2 font-[500] text-[#98A2B2]">
              Razmer görkezilýändigini ýa-da görkezilmeýändigini kesgitlemek
              üçin status düzüň
            </p>
          </div>
          <div className="flex justify-start w-[550px]">
            <Switch
              checked={size.is_active == 1 ? true : false}
              onChange={(event) =>
                setSize({
                  ...size,
        ...
        <div className="flex items-center border-t-[1px] justify-between py-[30px]">
          <div className="w-[380px]">
            <h1 className="text-[18px] font-[500]">Razmeri aýyr</h1>
            <p className="text-[14px] mt-2 font-[500] text-[#98A2B2]">
              Programmadan razmeri pozmak üçin
            </p>
          </div>
          <div className="flex justify-start w-[550px]">
            <button
      ...
      >
        ...
        >
          <div className="flex w-[350px] border-b-[1px] border-[#E9EBF0] pb-5 justify-between items-center">
            <h1 className="text-[20px] font-[500]">Razmer aýyrmak</h1>
          ...
          <div>
            <h1 className="text-[16px] text-center my-10 font-[400]">
              Razmeri aýyrmak isleýärsiňizmi?
            </h1>
      ...
      <div className="w-full mt-5 flex justify-between items-center bg-white py-4 px-5 border-[1px] border-[#E9EBF0] rounded-[8px]">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-[14px] font-[400]">
            {size?.updated_at?.slice(0, 10) +
              " / " +
              size?.updated_at?.slice(11, 16)}
          </h1>
        </div>
...
export default React.memo(SizeUpdate);
