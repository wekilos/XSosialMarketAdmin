...
const ColorCreate = () => {
  ...
  const creatColor = () => {
    ...
    color.title && formData.append("title", color.title);
    formData.append("is_active", color.is_active ? 1 : 0);
    file && formData.append("code", file);

    if (file?.length > 0 && color.title?.length > 0) {
      axiosInstance
        .post("/colors/create", formData)
        .then((data) => {
          setLoading(false);
    ...
    <div className="w-full">
      ...
      <div className="w-full min-h-[60vh] p-5 bg-white rounded-[8px]">
        <div className=" flex items-center gap-4 pb-5 border-b-[1px] border-b-[#E9EBF0]">
          ...
          <h1 className="text-[20px] font-[500]">Reňk goş</h1>
        </div>
        ...
        <div className="flex items-center border-t-[1px] justify-between py-[30px]">
          <div className="w-[380px]">
            ...
            <p className="text-[14px] mt-2 font-[500] text-[#98A2B2]">
              Reňkiň adyny girizeniňizde 30 simwoldan geçmäň.
            </p>
          </div>
          <div className="flex justify-start w-[550px]">
            <input
...
export default React.memo(ColorCreate);
